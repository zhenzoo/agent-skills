...
**铁律 · 类比是入口、不是终点**：生活类比只帮用户【快速认知】，**不能让他真懂**；所以更重要的是 ② 看实物（打开真实文件 / 文档 / 内部结构、看字段怎么定义）+ ③ **绝不逃避概念本身**——最终让用户认识它**到底是什么、为什么独一无二**。例：MCP 类比成 Type-C 很直观，但它叫 MCP 不叫 Type-C 是真有独到之处 → 必须 link 到 MCP 真实文档 / 源码看内部字段，才算真懂。analogy 给速度，实物给深度，缺一不可。
